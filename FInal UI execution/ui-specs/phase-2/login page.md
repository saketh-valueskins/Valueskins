...
> Template: **Login / Auth landing** — **dark default + light toggle** (v2).
> Governs: `BRANDING.md` is law. When this doc and the brand guide disagree, the brand guide wins.
> Status: **v2 — dark premium.** Sample: `_samples_delete_later/login-sample-v2.html`. §§1,3,4 (pill removal, single-viewport, no black band) still hold; the theme + life below supersede the old light-only treatment.
...
## 0b. v2 — dark premium elevation (the upgrade)

The plain light login read generic. v2 makes it the premium brand moment it should be. **Dark is the default** (BRANDING §6 / §10.1); a light toggle stays for parity.

1. **Dark surface** — tonal gradient `radial-gradient(60% 45% at 84% -8%, rgba(160,138,94,0.18), transparent 60%), linear-gradient(155deg,#0A0A0A,#161512 58%,#20201A)`; faint sand glow. Off-white text. (Light theme: off-white→sand gradient, near-black text.)
2. **Quiet life** — **drifting ValueSkin pixel identities** in the background (faint, `opacity ~0.09`, slow float), plus a **staggered entrance** for the wordmark → tagline → line → button → sign-up → trust → footer. Transform/opacity only; `prefers-reduced-motion` renders all static. Ties the brand identity in from the first screen.
3. **One confident line restored** (not filler): `Where creators and brands close deals on earned trust.` — `earned trust` in deep sand, weight 600, ~18–20px. States the product in one line. (This replaces the cut `One account for everything.` — it earns its place; filler did not.)
4. **Crisp single action** — `Continue with Google` is a **solid button** (off-white on dark / near-black on light) with the official Google `G` glyph, `10px` radius, soft tinted shadow, ~420px max, hover lift.
5. **Trust whisper** under the sign-up line: `Escrow-backed · Verified identities · Earned reputation` (small, muted, sand dot separators). Reassurance, not clutter.
6. **Slim footer** pinned to the bottom (hairline top, no black band): `© 2026 ValueSkins` left · `Terms · Privacy · Cookies` right. Keeps the single-viewport, no-scroll fit.
7. Order top→bottom, centered: wordmark · tagline · one-line · Google button · `Don't have an account? Sign up` · trust whisper · footer. Theme toggle top-right.

Everything else (no pill §1, single viewport §3, no black band §4, quiet copy) stays. The colour-mapping in §5 below is the **light-theme** reference; for dark use the tokens in §0b.1 and off-white text.
