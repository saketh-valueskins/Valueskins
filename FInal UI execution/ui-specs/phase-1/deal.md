...
- **Two-column layout (desktop):** a grid, `~1.35fr : 0.85fr`, gap `~22px`, `align-items:start`, inside a `max-width:1120px` wrap.
  - **LEFT column = the deal** (offer hero + terms + tags + progression).
  - **RIGHT column = the chat**, `position:sticky; top:88px` so it stays in view while the left scrolls.
  - **Collapses to a single column ≤900px** (offer stacks above chat; chat becomes `position:static`).
- **Offer hero (LEFT, DARK — the anchor):** `Offer from [brand]`, the **amount large**, a **net-after-fee line** (§4), and a one-line **escrow** explainer. Actions live here: **Accept · Counter · Decline**. `Counter` reveals a single number field (progressive disclosure). On accept → an `Agreed — escrow funded` state.
- **Terms strip (LEFT, under the offer — highlighted, scannable, not centerpiece):** `Deliverables · Compensation · Deliver by · Usage/Review` in a clean 4-up. This is what they're "getting into," visible at a glance.
- **Tag row (LEFT):** `Intent`, `Content type`, and `Match %` as small chips (§5).
- **Progression (LEFT):** thin animated stepper `Brief → Offer → Agreed → In progress → Paid` (sand/near-black, **never green**).
- **Chat (RIGHT column — clearly separate):** labeled "questions & rapport — not terms," with hint that money/terms are handled on the left. Gated until the creator accepts/counters. Messages animate in; **"logged with UTC timestamp"** note (the on-record moat).
