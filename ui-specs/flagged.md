...
## F9 — Leaked DB credential in committed .env.production (SECURITY — needs founder action)
- `marketplace/.env.production` (line 2) commits a **live `DATABASE_URL` with a real password** for the Render Postgres host `dpg-d8so9frsq97s73e31bq0-a.oregon-postgres.render.com:5432`.
- Severity: **medium, not emergency.** Repo is **private** (not publicly leaked), but the DB port **5432 is reachable from the public internet** — anyone with the password could connect from anywhere. Anyone with repo access has the password.
- **Action (founder only — needs Render dashboard, can't be done from code):**
  1. Rotate the DB password in Render.
  2. Move `DATABASE_URL` to Vercel dashboard env only (remove from committed `.env.production`).
  3. Restrict the DB to internal/allowlisted IPs if Render supports it.
  4. Consider scrubbing the secret from git history (it's in past commits).
- Discovered 2026-07-18 while fixing the OAuth `.env.production` override (same file). Related: the OAuth placeholders in this file were already fixed; this credential was left for founder to rotate safely.
