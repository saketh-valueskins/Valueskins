...
// Profile page — built to ui-specs/phase-2/Profile page.md. Every value here comes
// from that spec; the visual reference is phase-2/profile-page-mock.svg.
// Currency is intentionally absent (GP3 / flagged.md F1).
// Brand accounts keep their existing BrandProfile editor — this spec covers the
...
export default function ProfileMePage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/profile/me', { credentials: 'include' })
      ...
      .catch(() => { /* profile stays null; the page renders its zero state */ });
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: TOKENS.dark.bgA, color: TOKENS.dark.muted2, fontFamily: FONT }}>
        Loading…
      </div>
    );
  }
  ...
  const displayName = profile?.display_name || account?.display_name || 'Your profile';
  ...
  return (
    <>
      <Head><title>{displayName} · ValueSkins</title></Head>
