  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg }}>
      ...
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <h1 style={{ fontSize: '24px', fontWeight: 800, color: C.text, margin: 0, flex: 1 }}>Brands</h1>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                <th style={{ padding: '12px', textAlign: 'left', color: C.text, fontWeight: 600, fontSize: '14px' }}>Company Name</th>
                ...
                <th style={{ padding: '12px', textAlign: 'left', color: C.text, fontWeight: 600, fontSize: '14px' }}>Deals Posted</th>
                <th style={{ padding: '12px', textAlign: 'left', color: C.text, fontWeight: 600, fontSize: '14px' }}>Verification</th>
            ...
            <tbody>
              {filteredBrands.map((brand, idx) => (
                <tr key={idx} style={{ borderBottom: `1px solid ${C.border}`, background: idx % 2 === 0 ? C.surface : 'transparent' }}>
                  <td style={{ padding: '12px', color: C.text, fontSize: '14px' }}>{brand.company_name || 'N/A'}</td>
                  <td style={{ padding: '12px', color: C.text, fontSize: '14px' }}>{brand.email}</td>
                  <td style={{ padding: '12px', color: C.textSecondary, fontSize: '14px' }}>{new Date(brand.created_at).toLocaleDateString()}</td>
                  <td style={{ padding: '12px', color: C.text, fontSize: '14px' }}>{brand.total_deals || 0}</td>
