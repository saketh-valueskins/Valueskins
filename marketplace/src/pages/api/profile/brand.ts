...
async function ensureProfileColumns() {
  ...
  const cols = [
    ...
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS preferred_deal_types JSONB DEFAULT '[\"paid\"]'::jsonb",
    ...
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'brand'",
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'GET') {
      const profileResult = await query(
        `SELECT
          ...
          availability, response_time, pitch_text
    ...
    if (req.method === 'PUT' || req.method === 'PATCH') {
      ...
      const jsonbColumns = new Set(['languages', 'preferred_deal_types', 'modules']);
    ...
    console.error('Brand profile error:', error?.message || error);
